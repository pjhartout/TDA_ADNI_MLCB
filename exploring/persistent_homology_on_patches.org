...
PI = PersistenceImage(sigma=0.1, n_bins=100, weight_function=None, n_jobs=-1)
pi_cn = PI.fit_transform(diagrams_cubical_persistence_cn)
fig = (
    PI.plot(pi_cn)
    .update_layout(title="Persistence image for a CN patient")
    .write_html(
...
fig = (
    PI.plot(pi_mci)
    .update_layout(title="Persistence image for a MCI patient")
    .show()
)
...
fig = (
    PI.plot(pi_ad)
    .update_layout(title="Persistence image for a AD patient")
    .show()
)
...
fig = hk.plot(hk_cn).update_layout(title="Heat kernel for a CN patient").show()

hk_mci = hk.fit_transform(diagrams_cubical_persistence_mci)
fig = (
    hk.plot(hk_mci).update_layout(title="Heat kernel for a MCI patient").show()
)

hk_ad = hk.fit_transform(diagrams_cubical_persistence_ad)
fig = (
    hk.plot(hk_ad).update_layout(title="Heat kernel  for a AD patient").show()
)
