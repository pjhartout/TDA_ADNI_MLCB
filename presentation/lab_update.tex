...
\begin{center}
  ...
  \onslide<5->{
    \node[orange, below of=ph, yshift=-0.2cm](diag){\scriptsize Saliency assessment of topological features for diagnosis};
...
\end{table}
\onslide<2->$\rightarrow$ using a normal CPU, training  takes $<5$ minutes!
\end{frame}
...
\begin{frame}[fragile]{Limitations \& Outlook}
  \begin{itemize}
    ...
    \onslide<5-> \item Use other representations for prediction.
    \onslide<6-> \item Use topology-based embeddings for subtype identification?
  \end{itemize}
\end{frame}
