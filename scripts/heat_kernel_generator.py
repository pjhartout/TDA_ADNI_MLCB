...
"""heat_kernel_generator.py

This script generates the heat kernel for a given array, which represents
...
def get_heat_kernel(persistence_diagram):
    pi = HeatKernel(
        sigma=0.001, n_bins=N_BINS, n_jobs=N_JOBS
    )
...
def main():
    ...
    path_to_heat_kernel = DOTENV_KEY2VAL["DATA_DIR"] + "patch_91_heat_kernel/"

    if not os.path.exists(path_to_heat_kernel):
        os.mkdir(path_to_heat_kernel)

    patch_data, files_loaded = get_all_available_patches(path_to_patch)
    persistence_diagrams = cubical_persistence(patch_data)
    persistence_images = get_heat_kernel(persistence_diagrams)

    export_persistence_images(
        persistence_images, files_loaded, path_to_heat_kernel
    )
