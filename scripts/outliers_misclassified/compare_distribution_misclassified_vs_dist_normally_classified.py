...
with the distribution of distance values from the average persistence image
"""
...
def main():
    ...
    dest_dir_images = utils.make_dir(
    ...
    distance_files = [
        ...
        + "distance_from_median_pi_CN.csv",
        DOTENV_KEY2VAL["GEN_DATA_DIR"]
        + "/distance_from_median_image/"
        + "distance_from_median_pi_AD.csv",
    ]
    patient_types = [
        "CN",
        "AD",
    ...
    for distances, patient_type in zip(distance_files, patient_types):
        ...
        distances["misclassified"] = distances.isin(
            {"index": misclassification}
        )["index"]
    ...
    for patient in diagnoses.keys():
        diagnoses[patient] = [
            str(x) for x in diagnoses[patient].values() if x != "nan"
        ]

    for patient in diagnoses.keys():
        if len(set([str(x) for x in diagnoses[patient] if x != "nan"])) > 2:
    ...
    df = pd.read_csv(
        DOTENV_KEY2VAL["GEN_DATA_DIR"] + "misclassification.csv", usecols=[1]
    )
    ...
    print(
        f"Poportions of diagnoses for misclassified patients "
    ...
    print(
        f"Poportions of diagnoses for all patients"
